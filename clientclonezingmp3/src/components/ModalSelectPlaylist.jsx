...
function ModalSelectPlaylist({ showed, onHide, category, onRefresh }) {
  ...
  const handleSave = async () => {
    const fetchAddPlaylistToCategory = async (idPlaylist) => {
      await addPlaylistToCategory(category.id, idPlaylist);
    };
    await Promise.all(
      playlistSelected.map(async (item) => {
    ...
    <Modal show={showed} onHide={onHide}>
      ...
      <Modal.Body>
        <div className="input-search" style={{ width: "100%", height: "50px" }}>
          ...
          <input
            type="text"
            placeholder="Nhập tên playlist cần tìm"
            style={{ background: "none", border: "none", color: "white" }}
            onChange={handleFilterPlayList}></input>
        </div>
