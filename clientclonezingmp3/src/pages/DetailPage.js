...
import { addAlbum, getAlbumNew, getAlbumsByArtistId } from "../service/AlbumService";
...
function DetailArtist() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const data = await addAlbum(name,image,id);
      if (data) {
        setAlbumSave(data);
        handleChangeModal();
      }
  ...
  useEffect(() => {
    ...
    const fetchAlbum = async () => {
      try {
        const respone = await getAlbumsByArtistId(id);
        if (respone) {
          setAlbum(respone);
        }
      } catch (error) {
